...
def transform_data():
	...
	with open(f"{RAW}/sales.csv") as fin, open(f"{PROCESSED}/sales_clean.csv", "w") as fout:
		fout.write(fin.read())

def load_lakehouse():
	os.makedirs(CURATED, exist_ok=True)
	with open(f"{PROCESSED}/sales_clean.csv") as fin, open(f"{CURATED}/sales_curated.csv", "w") as fout:
		fout.write(fin.read())
...
with DAG( dag_id="bigdata_pipeline_complete", start_date=datetime(2024, 1, 1), schedule_interval="@daily", catchup=False, description="Pipeline Big Data complet orchestré avec Airflow") as dag :
